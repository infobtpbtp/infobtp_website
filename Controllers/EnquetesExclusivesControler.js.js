...
export default class EnquetesExclusivesControler {
    static async createEnquetesExclisives (req, res) {
        ...
        try {
            ...
            const enquetesExclusives = new EnquetesExclusives({
                titres: {
                    grandTitre, 
                    contenuGrandTitre, 
            ...
            let newItem = await enquetesExclusives.save();
            res.status(201).json({ message: 'Article créé avec succès', enquetesExclusives: newItem });
        } catch (error) {
    ...
    static async updateOne(req, res) {
        try {
            ...
            if (!updatedEnquetesExclusives) {
                return res.status(404).json({ error: "Article non trouvé" });
            }

            res.status(200).json({ message: 'Article mis à jour avec succès', enquetesExclusives: updatedEnquetesExclusives });
        } catch (error) {
            res.status(400).json({ error: error.message });
        }
    }
    ...
    static async deleteOne(req, res) {
        try {
            const articleId = req.params.id;
            const deletedEnqueteexclusives = await EnquetesExclusives.findByIdAndDelete(articleId);

            if (!deletedEnqueteexclusives) {
                return res.status(404).json({ error: "Article non trouvé" });
            }
