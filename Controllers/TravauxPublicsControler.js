import TravauxPublics from "../models/travauxPublics.js";
import PublicReportage from "../models/publicreportage.js";

export default class TravauxPublicsControler {
    static async createTravauxPublics (req, res) {
        ...
        try {
            ...
            const travauxPublics = new TravauxPublics({
                titres: {
                    grandTitre, 
                    contenuGrandTitre, 
            ...
            let newItem = await travauxPublics.save();
            res.status(201).json({ message: 'Article créé avec succès', travauxPublics: newItem });
        } catch (error) {
    ...
    static getOne(req, res) {
        ...
        TravauxPublics.findOne({ _id: articleId })
            .then(article => {
                if (!article) {
    ...
    static async updateOne(req, res) {
        try {
            ...
            const updatedTravauxPublics = await TravauxPublics.findByIdAndUpdate(articleId, updateData, { new: true });

            if (!updatedTravauxPublics) {
                return res.status(404).json({ error: "Article non trouvé" });
            }

            res.status(200).json({ message: 'Article mis à jour avec succès', travauxPublics: updatedTravauxPublics });
        } catch (error) {
            res.status(400).json({ error: error.message });
        }
    }
    ...
    static async deleteOne(req, res) {
        try {
            const articleId = req.params.id;
            const deletedTravauxpublics = await TravauxPublics.findByIdAndDelete(articleId);

            if (!deletedTravauxpublics) {
                return res.status(404).json({ error: "Article non trouvé" });
            }
