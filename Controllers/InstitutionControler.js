...
export default class InstitutionControler {
    static async createInstitutions (req, res) {
        ...
        try {
            ...
            let newItem = await institutions.save();
            res.status(201).json({ message: 'Article créé avec succès', institutions: newItem });
        } catch (error) {
    ...
    static async deleteOne(req, res) {
        try {
            const articleId = req.params.id;
            const deletedDivers = await Institutions.findByIdAndDelete(articleId);
