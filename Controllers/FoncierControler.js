import Foncier from '../models/foncier.js'
import PublicReportage from "../models/publicreportage.js";

export default class FoncierControler {
    static async createFoncier (req, res) {
        ...
        try {
            ...
            // URLs des image après upload sur Cloudinary
            ...
            let newItem = await foncier.save();
            res.status(201).json({ message: 'Article créé avec succès', foncier: newItem });
        } catch (error) {
    ...
    static async updateOne(req, res) {
        try {
            ...
            const updatedFoncier = await Foncier.findByIdAndUpdate(articleId, updateData, { new: true });

            if (!updatedFoncier) {
                return res.status(404).json({ error: "Article non trouvé" });
            }

            res.status(200).json({ message: 'Article mis à jour avec succès', Foncier: updatedFoncier });
        } catch (error) {
            res.status(400).json({ error: error.message });
        }
    }
    ...
    static async deleteOne(req, res) {
        try {
            const articleId = req.params.id;
            const deletedFoncier = await Foncier.findByIdAndDelete(articleId);
