...
export default class EconomieControler {
    static async createEconomie (req, res) {
        ...
        try {
            ...
            let newItem = await economie.save();
            res.status(201).json({ message: 'Article créé avec succès', economie: newItem });
        } catch (error) {
    ...
    static async updateOne(req, res) {
        try {
            ...
            res.status(200).json({ message: 'Article mis à jour avec succès', economie: updatedEconomie });
        } catch (error) {
            res.status(400).json({ error: error.message });
        }
    }
    ...
    static async deleteOne(req, res) {
        try {
            const articleId = req.params.id;
            const deletedEconomie = await Economie.findByIdAndDelete(articleId);
