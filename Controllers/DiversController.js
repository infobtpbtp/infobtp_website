...
export default class DiversController {
    ...
    static async updateOne(req, res) {
        try {
            ...
            const updatedDivers = await Divers.findByIdAndUpdate(articleId, updateData, { new: true });

            if (!updatedDivers) {
                return res.status(404).json({ error: "Article non trouvé" });
            }

            res.status(200).json({ message: 'Article mis à jour avec succès', divers: updatedDivers });
        } catch (error) {
            res.status(400).json({ error: error.message });
        }
    }
    ...
    static async deleteOne(req, res) {
        try {
            const articleId = req.params.id;
            const deletedDivers = await Divers.findByIdAndDelete(articleId);
